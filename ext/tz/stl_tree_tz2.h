...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  struct _Rb_tree_node_base
  {
    ...
    int _TZ_refCount;
    void* _TZ_tree;
    bool _TZ_destroyPrepared;
    
    _Rb_tree_node_base() : _TZ_refCount(0), _TZ_destroyPrepared(false) { }
    ~_Rb_tree_node_base(){
    	_M_left = this;
    	_M_right = this;
    	_M_parent = 0;
  ...
  template<typename _Tp>
    struct _Rb_tree_iterator
    {
      ...
      ~_Rb_tree_iterator(){
    	if(_M_node){
			_M_node->_TZ_refCount -= 1;
			if(_M_node->_TZ_refCount == 0 && _M_node->_TZ_destroyPrepared){
				delete _M_node;
			}
    	}
      }
  ...
  template<typename _Key, typename _Val, typename _KeyOfValue,
    ...
    protected:
	  ...
	  void _TZ_simple_delete(_Rb_tree_node_base* __p){
		  delete __p;
	  }
	  ~_Rb_tree_impl(){
