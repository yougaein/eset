...
#ifndef _STL_TREE_TZ_H
#define _STL_TREE_TZ_H 1
...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  template<typename _Key, typename _Val, typename _KeyOfValue,
    ...
    protected:
	  ...
	  _Rb_tree_impl()
	  : _Node_allocator(), _M_key_compare(), _M_header(*new _Rb_tree_node_base),
	    _M_node_count(0)
	  { _M_initialize(); }

    static void destroyNode(_Rb_tree_node_base* p){ 
      _Node_allocator().destroy((_Link_type)p); 
      _Node_allocator().deallocate((_Link_type)p, 1); 
    }

	  static void _TZ_simple_delete(_Rb_tree_node_base* __p){
		  delete __p;
	  }
	  ~_Rb_tree_impl(){
		  _Rb_tree_node_base* __p = &_M_header;
		if(__p->_TZ_refCount == 0){
				delete __p;
		}else{
			__p->prepareDestroy(&_TZ_simple_delete);
		}
	  }
